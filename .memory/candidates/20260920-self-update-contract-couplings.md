...
- scripts/install.sh still resolves the installed version through the GitHub /releases/latest redirect, which is separate from the daemon's own signed-BOM discovery and is not covered by the BOM trust chain.
