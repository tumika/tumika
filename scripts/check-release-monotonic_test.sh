#!/usr/bin/env bash
# Fixture tests for check-release-monotonic.sh.
#
...
# No published release at all: the first release of every component.
write_list no-previous
write_release "$WORK/no-previous/release.yaml" 2026.09.01 "daemon 0.0.1"
expect_pass no-previous "no published release to compare against"

# The ordinary pass: the version advances.
...
expect_pass greater "daemon 0.0.1 -> 0.0.2"

# Repeating a component version publishes a head no client moves onto.
...
expect_fail equal "must compare greater than the last published one"
...
write_release "$WORK/missing-asset/release.yaml" 2026.09.01 "daemon 0.0.2"
expect_pass missing-asset "carries no release.yaml asset"
...
# A component the previous release did not name has nothing to advance past.
...
expect_pass new-component "desktop 0.0.1 is new since v2026.08.00"
